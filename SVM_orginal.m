...
%% Normalize Data (Min-Max normalization to range [-1, 1])
minVals = min(sigpad, [], 2);
maxVals = max(sigpad, [], 2);
rangeVals = maxVals - minVals;

rangeVals(rangeVals == 0) = 1;

sigpad = 2 * ((sigpad - minVals) ./ rangeVals) - 1;
...
elapsedTime = toc;
disp(['Total Run Time: ', num2str(elapsedTime), ' seconds']);
