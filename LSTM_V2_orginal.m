...
% Create label
pathToSignals = "D:\UNSW\Thesis data (orginal)\data500ns2class";
...
cv1 = cvpartition(labels, 'HoldOut', 0.1); 
...
cv2 = cvpartition(YtrainCV, 'HoldOut', 0.1111); % 15/85 ≈ 0.1765
...
for i = 1:length(hiddenSizes)
    ...
    % Define the LTSM layers
     layers = [ ...
        ...
        lstmLayer(hiddenSize, 'OutputMode', 'sequence')  % Adjusted LSTM hidden size
        dropoutLayer(0.2)                            % Dropout tuned
        lstmLayer(hiddenSize*2, 'OutputMode', 'last')    % Scaled hidden size
        dropoutLayer(0.2)                            % Dropout tuned
    ...
    % Confusion Matrix (no figure shown)
    confMat = confusionmat(Ytest, Ypred);
    
    % Display Results
    disp('Confusion Matrix of Simple Model:');
    rowNames = strcat("Predicted_", categories(Ytest)');
    colNames = strcat("Actual_", categories(Ytest)');
